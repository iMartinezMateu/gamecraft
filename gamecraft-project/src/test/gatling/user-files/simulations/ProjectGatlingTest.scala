...
class ProjectGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Project entity")
        ...
        .repeat(2) {
            ...
            .get("/gamecraftproject/api/projects")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new project")
            .post("/gamecraftproject/api/projects")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "projectName":"SAMPLE_TEXT", "projectDescription":"SAMPLE_TEXT", "projectWebsite":"SAMPLE_TEXT", "projectLogo":"SAMPLE_TEXT", "projectArchived":null}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_project_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created project")
                .get("/gamecraftproject${new_project_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created project")
            .delete("/gamecraftproject${new_project_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
