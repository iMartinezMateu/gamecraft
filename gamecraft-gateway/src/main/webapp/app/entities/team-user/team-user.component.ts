...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/team-user'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/team-user', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: TeamUser) {
        return item.id;
    }
    registerChangeInTeamUsers() {
        this.eventSubscriber = this.eventManager.subscribe('teamUserListModification', (response) => this.loadAll());
    }
