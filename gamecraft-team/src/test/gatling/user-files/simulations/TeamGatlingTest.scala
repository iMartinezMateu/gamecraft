...
class TeamGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Team entity")
        ...
        .repeat(2) {
            ...
            .get("/gamecraftteam/api/teams")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new team")
            .post("/gamecraftteam/api/teams")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "teamName":"SAMPLE_TEXT", "teamDescription":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_team_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created team")
                .get("/gamecraftteam${new_team_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created team")
            .delete("/gamecraftteam${new_team_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
